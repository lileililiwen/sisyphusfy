...
- 13 changes archived: minimal-runner, durable-iteration-loop, agent-adapters-and-model-fallback, workflow-integrations, optional-archive-commit-hooks, audit-and-fix-agent-workflow, repair-final-workflow-safety, improve-human-friendly-cli, add-distribution-and-installers, repair-distribution-release-gates, close-audit-identified-gaps, simplify-verification-failures, stream-progress-with-explicit-timing.
...
- 519 tests across 41 test files: `tests/test_progress.py` covers duration formatting, component-labelled streaming, idle heartbeats, timeout reporting, interruption, bounded output, and clean JSON. `ruff check` clean; `compileall` clean; `openspec validate --all --strict` passes.
