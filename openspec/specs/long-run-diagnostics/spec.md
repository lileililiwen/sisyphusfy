...
## Purpose
Long-running use needs concise, correctly labelled diagnostics and
practical controls. Agent failures must not be confused with
verification log naming, high-level `run`/`resume` must expose
verification-timeout overrides, and verbose inspection must read saved
evidence without rerunning work. Human output stays short; full
streams remain opt-in.
