...
## Purpose
Every managed subprocess (agent, verification, hook, workflow check) runs
through one bounded execution primitive. The primitive bounds in-memory
output, writes a complete local diagnostic log when diagnostics are
enabled, reaps descendants on timeout or interrupt, and classifies
success, failure, timeout, interrupt, and command-not-found outcomes so
callers do not handle an unstructured exception.
